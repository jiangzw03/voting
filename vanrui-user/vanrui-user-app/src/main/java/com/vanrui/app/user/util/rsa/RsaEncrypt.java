...
public class RsaEncrypt {
	...
	/** 保存生成的密钥对的文件名称。 */
	private static final String RSA_PAIR_FILENAME = PropertiesLoader.getProperty("WINDOWS.RSA.PAIR.FILENAME");
	/*私钥字符串*/
	...
	private static synchronized KeyPair generateKeyPair() {
		try {
			keyPairGen.initialize(KEY_SIZE, new SecureRandom("20170308".getBytes()));
	...
	private static String getRSAPairFilePath() {
		String urlPath = RsaEncrypt.class.getResource("/").getPath();
...
//		 return (new File(urlPath).getParent() + RSA_PAIR_FILENAME);
	...
	public static void main(String[] args) {
		 KeyPair st= getKeyPair();
		 saveKeyPair(st);
//		KeyPair st = readKeyPair();
//		System.out.println(st);
	 ...
	 * @return
	 */
	public static String decryptStringAndBack(String encrypttext) {

		System.out.println(RSA_PAIR_FILENAME);
		...
		encrypttext = encrypttext.replace(" ", "+");
		try {
			password = decryptString(encrypttext);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static String decryptString(String encrypttext) {
		...
		String strpk = new String(Base64.getEncoder().encodeToString(pubKey.getEncoded()));
		String strprivk = new String(Base64.getEncoder().encodeToString(privKey.getEncoded()));*/
		 ...
		 * 过时的方法
		 * try {
			priPKCS8 = new PKCS8EncodedKeySpec(new BASE64Decoder().decodeBuffer(STR_PRIV_KEY));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}*/
